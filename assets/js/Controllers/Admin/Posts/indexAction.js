/**
 * Posts managenent page JS for its controller.
 */
            ...
            let dataTable = $(thisClass.datatableIDSelector).DataTable({
                ...
                'columnDefs': [
                    ...
                    },
                    {
                        ...
                        'targets': 5,
                        'render': function(data, type, row, meta) {
                            let categoriesHtmlArray = [];
                            if (row.categories) {
                                row.categories.forEach(function(item, index) {
                                    categoriesHtmlArray.push('<a href="' + RdbCMSAPostsIndexObject.editCategoryUrlBase + '/' + item.tid + '">' + item.t_name + '</a>');
                                });
                            }
                            return '<small>' + categoriesHtmlArray.join(', ') + '</small>';
                        }
                    },
                    {
                        'data': 'tags',
                        ...
                        'render': function(data, type, row, meta) {
                            ...
                            if (row.tags) {
                                row.tags.forEach(function(item, index) {
                                    tagsHtmlArray.push('<a href="' + RdbCMSAPostsIndexObject.editTagUrlBase + '/' + item.tid + '">' + item.t_name + '</a>');
                                });
                            }
                            return '<small>' + tagsHtmlArray.join(', ') + '</small>';
                        }
                    },
                    {
                        'data': 'post_add',
    ...
    ajaxGetFiltersValue() {
        ...
        .then(function(responseObject) {
            ...
            if (typeof(response) !== 'undefined') {
                ...
                if (typeof(response.allCategories) !== 'undefined') {
                    let optionString = '';
                    response.allCategories.forEach(function(item, index) {
                        optionString += '<option value="' + item.value + '">';
                        if (item.level && item.level > 1) {
                            let space = ' &nbsp; ';
                            optionString += space.repeat((item.level - 1));
                        }
                    ...
                    thisForm.querySelector('#rdba-filter-tid').innerHTML = optionString;
                }
