...
class IndexController extends \Rdb\Modules\RdbCMSA\Controllers\Admin\RdbCMSAdminBaseController
{
    ...
    public function doGetFiltersAction(): string
    {
        ...
        // get categories
        $output['allCategories'] = $PostsSubController->getCategoriesForSelectbox();
    ...
    protected function doGetItems(array $configDb): array
    {
        ...
        if (isset($result['items']) && is_iterable($result['items'])) {
            foreach ($result['items'] as $row) {
                $row->languages = $this->getLanguagesAndTranslationMatched($row, $languages, $TranslationMatcherDb, 'posts', 'post_id');
            }// endforeach;
            unset($row);
        }
    ...
    public function indexAction(): string
    {
        ...
        $output['pageTitle'] = d__('rdbcmsa', 'Posts');
        ...
        if ($this->Input->isNonHtmlAccept() || $this->Input->isXhr()) {
            ...
            $Assets->addMultipleAssets('css', ['rdbcmsaPostsIndexAction'], $Assets->mergeAssetsData('css', $moduleAssetsData, $rdbAdminAssets));
            $Assets->addMultipleAssets('js', ['rdbcmsaPostsIndexAction'], $Assets->mergeAssetsData('js', $moduleAssetsData, $rdbAdminAssets));
            $Assets->addJsObject(
                'rdbcmsaPostsIndexAction',
            ...
            $output['pageContent'] = $this->Views->render('Admin/Posts/index_v', $output);
