...
class EditController extends \Rdb\Modules\RdbCMSA\Controllers\Admin\RdbCMSAdminBaseController
{
    ...
    public function indexAction(string $post_id): string
    {
        ...
        $output = [];
        $output['configDb'] = $this->getConfigDb();
        $output['pageTitle'] = d__('rdbcmsa', 'Edit post');
        ...
        if ($this->Input->isNonHtmlAccept()) {
            ...
            $Assets->addMultipleAssets('js', ['rdbcmsaPostsEditAction'], $Assets->mergeAssetsData('js', $moduleAssetsData, $rdbAdminAssets));
            $Assets->addJsObject(
                'rdbcmsaPostsEditAction',
