...
class AddController extends \Rdb\Modules\RdbCMSA\Controllers\Admin\RdbCMSAdminBaseController
{
    ...
    {
        // processing part ----------------------------------------------------------------------------------------------------
        $this->checkPermission('RdbCMSA', 'RdbCMSAPages', ['add']);
    ...
    public function indexAction(): string
    {
        ...
        $output = [];
        $output['configDb'] = $this->getConfigDb();
        $output['pageTitle'] = d__('rdbcmsa', 'Add new page');
        ...
        if ($this->Input->isNonHtmlAccept()) {
            ...
            $Assets->addMultipleAssets('js', ['rdbcmsaPagesAddAction'], $Assets->mergeAssetsData('js', $moduleAssetsData, $rdbAdminAssets));
            $Assets->addJsObject(
                'rdbcmsaPagesAddAction',
